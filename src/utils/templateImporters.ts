...
let PSD: { fromBuffer: (b: Uint8Array) => { parse: () => void; tree: () => { get: (k: string) => unknown; children?: unknown[]; toPng?: () => string } } } | null = null;
try {
  import('psd').then(module => {
    PSD = module.default || module;
  }).catch(() => {
    // PSD library not available
  });
} catch {
  // PSD library init failed
}
...
export const importWordDocument = async (file: File): Promise<ImportResult> => {
  try {
      ...
      if (text && text.length > 0) {
        ...
        elements.push({
          ...
          properties: {
            ...
            textAlign: (computedStyle.textAlign as 'left' | 'center' | 'right') || 'left',
    ...
    return {
      ...
      errors: ['Failed to import Word document'],
    };
  }
};
...
export const importPhotoshopDocument = async (file: File): Promise<ImportResult> => {
  try {
    ...
    // Extract layers
    const extractLayers = (node: { children?: Record<string, unknown>[], get: (k: string) => unknown, toPng: () => string, type?: string }, parentX = 0, parentY = 0, zIndex = 0): void => {
      if (node.children) {
        node.children.forEach((child: { children?: Record<string, unknown>[], get: (k: string) => unknown, toPng: () => string, type?: string }, index: number) => {
          const layerX = (child.get('left') as number || 0) + parentX;
          const layerY = (child.get('top') as number || 0) + parentY;
          const layerWidth = (child.get('width') as number) || 100;
          const layerHeight = (child.get('height') as number) || 100;
          const layerOpacity = ((child.get('opacity') as number) || 255) / 255;
          ...
          if (child.get('type') === 'layer' && layerVisible) {
            ...
            if (imageData) {
              elements.push({
                ...
                x: layerX as number,
                y: layerY as number,
                width: layerWidth as number,
                height: layerHeight as number,
                opacity: layerOpacity as number,
                visible: layerVisible as boolean,
                zIndex: zIndex + index,
                properties: {
                  src: imageData,
                },
            ...
            extractLayers(child as { children?: Record<string, unknown>[], get: (k: string) => unknown, toPng: () => string, type?: string }, layerX as number, layerY as number, zIndex + index);
          }
        });
      }
    };
    ...
    return {
      ...
      elements: [],
      canvas: { width: 400, height: 600 },
      errors: ['Failed to import Photoshop document'],
    };
  }
};
...
export const importImageFile = async (file: File): Promise<ImportResult> => {
  try {
    ...
    return {
      ...
      elements: [],
      canvas: { width: 400, height: 600 },
      errors: ['Failed to import image file'],
    };
  }
};

// PDF Document Importer
export const importPDFDocument = async (file: File): Promise<ImportResult> => {
  try {
    // Dynamically import pdfjs-dist to avoid SSR issues if any
    ...
    // For now, we'll import the first page as a background
    const pageNum = 1;
    ...
    await page.render({
      ...
      viewport: viewport,
      canvas: canvas
    } as unknown as import('pdfjs-dist/types/src/display/api').RenderParameters).promise;
    ...
    return {
      ...
      elements: [],
      canvas: { width: 400, height: 600 },
      errors: ['Failed to import PDF document'],
    };
  }
};
...
export const convertToKonvaFormat = (elements: ImportedElement[]): Record<string, unknown>[] => {
  return elements.map(element => {
    const konvaElement: Record<string, unknown> = {
      id: element.id,
      x: element.x,
      y: element.y,
      width: element.width,
...
export const convertToFabricFormat = (elements: ImportedElement[]): Record<string, unknown>[] => {
  return elements.map(element => {
    const fabricElement: Record<string, unknown> = {
      id: element.id,
      left: element.x,
      top: element.y,
      width: element.width,
